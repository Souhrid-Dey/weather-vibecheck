...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function displayError(container, message) {
    container.innerHTML = `<p style="color: var(--accent-emergency)">❌ Error: ${message}</p>`;
  ...
  vibeForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      html += `<p style="font-size:1.1rem; margin-bottom:1rem; padding: 10px; border-left: 4px solid var(--accent-safe); background: var(--surface-hover)">"${plan.greeting_and_tone}"</p>`;
      
      html += `<h4>⚠️ Immediate Actions</h4><ul>`;
      plan.immediate_actions.forEach(a => html += `<li><strong>${a.task}</strong>: ${a.why}</li>`);
      html += `</ul>`;

      html += `<h4>☔ Pre-Storm Checklist</h4><ul>`;
      plan.pre_storm_checklist.forEach(t => html += `<li>${t}</li>`);
      html += `</ul>`;

      html += `<h4>🛑 During Storm Rules</h4><ul>`;
      plan.during_storm_rules.forEach(t => html += `<li>${t}</li>`);
      html += `</ul>`;

      html += `<p><strong>Contacts Suggestion:</strong> ${plan.emergency_contacts_suggestion}</p>`;
      ...
      ['day_1', 'day_2', 'day_3'].forEach(day => {
        if(plan[day]) {
          html += `<div style="margin-bottom:1rem; padding:10px; border:1px solid var(--border-color); border-radius:4px;">
            <h4 style="color:var(--accent-warning); text-transform:capitalize">${day.replace('_', ' ')}</h4>
            <p><strong>Breakfast:</strong> ${plan[day].breakfast}</p>
            <p><strong>Lunch:</strong> ${plan[day].lunch}</p>
            <p><strong>Dinner:</strong> ${plan[day].dinner}</p>
          </div>`;
        }
      });
  ...
  commuteForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      html += `<p style="font-size:1.2rem; color:${riskColor}; font-weight:bold; margin-bottom:1rem">Risk Level: ${route.risk_level}</p>`;
      
      html += `<h4>🌊 Primary Hazards</h4><ul>`;
      route.primary_hazards.forEach(h => html += `<li>${h}</li>`);
      html += `</ul>`;

      html += `<p><strong>Alternatives:</strong> ${route.alternative_suggestions}</p>`;
      html += `<div style="margin-top:1rem; padding:1rem; background:var(--surface-hover); border-left:4px solid ${riskColor}">
        <strong>Final Decision:</strong> ${route.go_no_go_decision}
      </div>`;
    ...
    try {
      ...
      langs.forEach(l => {
        html += `<div style="margin-bottom:1rem">
          <h4>${l.name}</h4>
          <textarea readonly rows="3" style="font-size:1rem; cursor:pointer" onclick="this.select()">${l.text}</textarea>
        </div>`;
      });

      html += `<p><strong>🔋 Tip:</strong> ${sos.advice}</p>`;
